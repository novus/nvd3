define(['sg/jquery','d3.v2.min'],function(jQuery){
...
nv.models.legend = function() {
      ...
      var margin = {top: 0, right: 0, bottom: 0, left: 0}
        ...
        , getCurrent = function(d) { return d3.round(d.values.reverse()[0][1]) }
        , color = nv.utils.defaultColor()
        , align = true
      ...
      function chart(selection) {
        selection.each(function(data) {
          ...
          seriesEnter.append('text')
              ...
              .attr('dy', '-10')
              .attr('dx', '0')
              .attr('fill',function(d,i){ return d.color || color(d,i) })
              .attr('class','incident-current');
          seriesEnter.append('text')
              .attr('text-anchor', 'start')
              .attr('dy', '2')
              .attr('dx', '0')
              .attr('class','incident-key');
          series.classed('disabled', function(d) { return d.disabled });
          series.exit().remove();
          // series.select('circle')
          //     .style('fill', function(d,i) { return d.color || color(d,i)})
          //     .style('stroke', function(d,i) { return d.color || color(d, i) });
          series.select('text.incident-key').text(getKey);
          series.select('text.incident-current').text(getCurrent);

          series
              .attr('transform', function(d, i) {
                var row = (height+14)/4;
                return 'translate(9,' + (height - (row*(i+1)) + (row)) + ')';
              });
...
nv.models.stackedAreaChart = function() {
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      if (showLegend) {
        ...
        if (incidentSpans) {
          // dump the legend's text nodes for description into a list
          var list = g.selectAll('.incident-key');
          // required for positioning math and element index
          var count = 0; 
          list.each(function(){
            // text value of the original svg text node in the legend
            var t = $(this).text();
            //position each at correct height going up from bottom
            var top = (122 - (count * 36)) + "px";  
            // prevent overlap of elements so tooltips work properly
            var bottom = (count * 36) + "px"; 
            // work through the four span nodes as the loop cycles
            var $span = $('#company_incidents > div > span:eq('+ count +')');
            // assign the span the label text, tooltip, and css offsets
            $span.html(t).attr('title', t).css({'top': top, 'bottom': bottom});
            // delete the svg text node
            $(this).remove(); 
  ...
  chart.incidentSpans = function(_) {
    if (!arguments.length) return incidentSpans;
    incidentSpans = _;
    return chart;
  };  
