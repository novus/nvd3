  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      if (stacked)
          bars.transition()
            .delay(function(d,i) { return i * delay / data[0].values.length })
            .attr('y', function(d) { return y((stacked ? d.y1 : 0)) })
            .attr('height', function(d) {
                  return Math.max(Math.abs(y(d.y + (stacked ? d.y0 : 0)) - y((stacked ? d.y0 : 0))),1)
            })
            .attr('x', function(d) { return stacked ? 0 : (d.series * x.rangeBand() / data.length ) })
            .attr('width', x.rangeBand() / (stacked ? 1 : data.length) );
      else
          bars.transition()
            .delay(function(d,i) { return i * delay/ data[0].values.length })
            .attr('x', function(d) { return d.series * x.rangeBand() / data.length })
            ...
            .attr('height', function(d,i) { return Math.max(Math.abs(y(getY(d,i)) - y(0)),1) || 0 });
