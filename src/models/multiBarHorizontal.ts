...
nv.models.multiBarHorizontal = function () {
    ...
    var margin = { top: 0, right: 0, bottom: 0, left: 0 }
        ...
        , getYerr = function (d, _i) { return d.yErr }
    ...
    function chart(selection) {
        ...
        selection.each(function (data: any[]) {
            ...
            y.domain(yDomain || d3.extent(d3.merge(seriesData).map(function (d: any) { return stacked ? (d.y > 0 ? d.y1 + d.y : d.y1) : d.y }).concat(forceY)))
            ...
            var groups = wrap.select('.nv-groups').selectAll('.nv-group')
                .data(function (d) { return d }, <any>function (_d, i) { return i });
            ...
            var barsEnter = bars.enter().append('g')
                .attr('transform', function (d: any, i, j) {
                    return 'translate(' + y0(stacked ? d.y0 : 0) + ',' + (stacked ? 0 : (j * x.rangeBand() / data.length) + <any>x(getX(d, i))) + ')'
                });
            ...
            if (stacked)
                ...
                (<any>bars).watchTransition(renderWatch, 'multibarhorizontal: bars')
                    .attr('transform', function (d, i) {
                        ...
                        const tx = (getY(d, i) < 0 ? y(getY(d, i)) : y(0));
                        const ty = (d.series * x.rangeBand() / data.length + <number>x(getX(d, i)));
