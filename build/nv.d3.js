...
nv.models.multiBar = function () {
    ...
    , container = null, getX = function (d) { return d.x; }, getY = function (d) { return d.y; }, forceY = [0] // 0 is forced by default.. this makes sense for the majority of bar graphs... user can always do chart.forceY([]) to remove
    ...
    function chart(selection) {
        ...
        selection.each(function (data) {
            ...
            if (showValues && !stacked) {
                ...
                bars = groups.selectAll('rect.nv-bar')
                    .data(function (d) { return (hideable && !data.length) ? hideable.values : d.values; });
                bars.exit().remove();
                barsEnter = bars.enter().append('rect')
                    .attr('class', function (d, _i) { return getY(d) < 0 ? 'nv-bar negative' : 'nv-bar positive'; })
                    .attr('x', function (_d, _i, j) {
                    return stacked && !data[j].nonStackable ? 0 : (j * x.rangeBand() / data.length);
                })
                    .attr('y', function (d, _i, j) { return y0(stacked && !data[j].nonStackable ? d.y0 : 0) || 0; })
                    .attr('height', 0)
                    .attr('width', function (_d, _i, j) { return x.rangeBand() / (stacked && !data[j].nonStackable ? 1 : data.length); })
                    .attr('transform', function (d, _) { return 'translate(' + x(getX(d)) + ',0)'; });
            }
            barsEnter.append('text');
            bars
            ...
            var barSelectionBase = bars;
            if (showValues && !stacked) {
                barSelectionBase = bars.selectAll('rect');
            }
            var barSelection = barSelectionBase.watchTransition(renderWatch, 'multibar', Math.min(250, duration))
                .delay(function (_, i) {
...
nv.models.multiBarHorizontal = function () {
    ...
    //------------------------------------------------------------
    var margin = { top: 0, right: 0, bottom: 0, left: 0 }, width = 960, height = 500, id = Math.floor(Math.random() * 10000) //Create semi-unique ID in case user doesn't select one
    , container = null, x = d3.scale.ordinal(), y = d3.scale.linear(), getX = function (d) { return d.x; }, getY = function (d) { return d.y; }, getYerr = function (d) { return d.yErr; }, forceY = [0] // 0 is forced by default.. this makes sense for the majority of bar graphs... user can always do chart.forceY([]) to remove
